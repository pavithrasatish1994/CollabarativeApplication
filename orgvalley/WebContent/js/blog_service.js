...
angular.module('myApp').factory('BlogService', ['$http', '$q', function($http, $q){
    ...
    function fetchAllBlogs() {
        ...
        $http.get('http://localhost:8080/OrgValleyBackend/blogs')
            .then(
            function (response) {
    ...
    function addCom(comment) {
        var deferred = $q.defer();
        $http.post('http://localhost:8080/OrgValleyBackend/cmnt/insert/',comment)
    ...
    function fetchAllCom(bid) {
        var deferred = $q.defer();
        $http.get('http://localhost:8080/OrgValleyBackend/coments/'+bid)
            .then(
            function (response) {
            	console.log('Got data');
            	deferred.resolve(response.data);
